...
- **Static reference data** (real-ish cities, airport/airline codes, plane types, etc.) is committed as source data (e.g. CSV under a scenario's folder) and loaded into the scenario's DB by a build script (e.g. `npm run db:build:travel`) — this is enumerable, factual vocabulary, not fabricated
...
- "No seed data" refers to this transactional layer only — it does not mean the DB is empty; a scenario's reference tables are populated ahead of time from its committed static data
