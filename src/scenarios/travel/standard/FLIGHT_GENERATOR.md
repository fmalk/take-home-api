...
- The legacy flat `Flight.price` field is `derivePrice()`: the USD price of the cheapest tier available on the Flight, walking `PRICE_TIER_ORDER = ['regular', 'economy', 'businessClass', 'firstClass']`; it only ever reads a Flight's `USD` pricing row, so LOY (below) never affects it.
- **Loyalty Points (TAK-28, v4 only)**: `Airline.hasLoyaltyProgram` now drives a `LOY` pricing row, gated by an `includeLoyalty` flag threaded through `findDirectFlights`/`findConnectingRoutes`/`makeFlight`/`makePricing` — every non-v4 call site omits it and gets the pre-TAK-28 pricing shape unchanged. When an airline has a loyalty program and the flag is set, one extra `FlightPricing` entry per seat class is added with `currency: 'LOY'`: `regular` costs 1000 LOY per USD of that class's base price, `economy` 800, `businessClass` 2500, `firstClass` 4500. The result is rounded up to the nearest 1000 (`LOYALTY_PRICE_STEP`) and clamped to `[LOYALTY_MIN_PRICE, LOYALTY_MAX_PRICE]` = `[9000, 1,000,000]`. `LOY` is excluded from `aggregateRouteMinimumPricing`'s per-currency `minimum` rollup (`legMinimumPriceByCurrency` skips it outright) — it's a parallel redemption price, never a competing "cheapest fare".
